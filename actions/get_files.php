<?php
	...
	*/
	...
	if(count($files) === 0) {
		...
		$html .= '<div class="control-group"><div class="controls"><a id="delete-files" class="btn disabled"><i class="icon-remove"></i> Delete Selected</a></div></div><table class="table table-striped table-hover table-bordered table-condensed"><thead><tr><th><input type="checkbox" id="files-delete-all-check" /></th><th>&nbsp;</th><th>Name</th><th>Type</th><th>Size</th><th>Added</th></tr></thead><tbody>';
		
		foreach($files as $file) {
			$html .= '<tr id="' . $file['_id'] . '" class="file"><td><input type="checkbox" class="file-delete-check" value="' . $file['_id'] . '" /></td><td><a href="' . Links::render("download-file", array($file['_id'])) . '" class="btn btn-mini"><i class="icon-download-alt"></i></a></td><td><a href="' . Links::render("view-file", array($file['_id'])) . '">' . $file['real_filename'] . '</a></td><td><span class="badge badge-info">';
